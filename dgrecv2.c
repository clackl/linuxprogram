...
void replay_to_sender(int, char*, struct sockaddr_in *, socklen_t);
...
int main(int ac, char *av[])
{
	...
	while ((msglen = recvfrom(sock, buf, BUFSIZ, 0, &saddr, &saddrlen)) > 0) {
		...
		replay_to_sender(sock, buf, &saddr, saddrlen);
	}
	return 0;
}

void replay_to_sender(int sock, char* msg, struct sockaddr_in * addrp, socklen_t len)
{
	char replay[BUFSIZ + BUFSIZ];
	sprintf(replay, "Thanks for your %d char message\n", strlen(msg));
	sendto(sock, replay, strlen(replay), 0, addrp, len);
}
